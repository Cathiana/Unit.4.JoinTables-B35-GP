...
const createUser = async({ username, password }) => {
    ...
    RETURNING * 
    `;
    const response = await person.query(SQL, [uuid.v4(), username, password])
...
const fetchUsers = async() => {
    ...
    FROM users
    `;
    const response = await person.query(SQL)
...
const createSkill = async({ name }) => {
    ...
    RETURNING * 
    `;
    const response = await person.query(SQL, [uuid.v4(), name])
    return response.rows[0];
};
...
const createUserSkill = async({ user_id, skill_id }) => {
    ...
    RETURNING * 
    `;
    const response = await person.query(SQL, [uuid.v4(), user_id, skill_id])
    return response.rows[0];
};
...
const fetchUserSkills = async(user_id) => {
    ...
    WHERE user_id = $1
    `;
    const response = await person.query(SQL, [ user_id ])
    return response.rows;
};
...
const destroyUserSkill = async({ user_id, id}) => {
    const SQL = `
     ...
     RETURNING *
    `;
   const response = await person.query(SQL, [ id, user_id ]);
